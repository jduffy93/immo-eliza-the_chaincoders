...
class ImmowebScraper:
    ...
    def get_listing_details(self, url: str) -> None:
        ...
        if (
            req.status_code != 200
        ):  # Checks whether url is available, and prints/saves to output file
            print(
                f"{req.status_code}: Website could not be reached! Here is the Url: {url}"
            )
        ...
        # Immoweb ID:
        for elem_id in soup.find_all(
            "div", attrs={"class": "classified__header--immoweb-code"}
        ):
            property_details["Property_ID"] = re.sub(
                r"\D", "", elem_id.text.strip()
            )  # Extract only digits
        ...
        for elem_price in soup.find_all("p", attrs={"class": "classified__price"}):
            ...
            property_details["Price"] = elem_price.find(
                "span", attrs={"class": "sr-only"}
            ).text.rstrip("€")
        ...
        # Type of property:
        if (
            property_details["Subtype_of_property"] == "House"
            or property_details["Subtype_of_property"] == "Apartment"
        ):
            property_details["Type_of_property"] = property_details[
                "Subtype_of_property"
            ]
        else:
        ...
        for row in soup.find_all("tr", attrs={"class": "classified-table__row"}):
            ...
            if key_element:
                key = key_element.contents[
                    0
                ].strip()  # Extract the key text and strip any leading/trailing whitespace
                value_element = row.find(
                    "td"
                )  # Find the value element (the table data)

                if value_element:
                    value = value_element.contents[
                        0
                    ].strip()  # Extract the value text and strip any leading/trailing whitespace
                    property_details[key] = (
                        value  # Add the key-value pair to the property details dictionary
                    )
    ...
    def get_page_url(self, page: int) -> list:
        ...
        if (
            page_req.status_code != 200
        ):  # Checks whether url is available, and prints/saves to output file
            print(
                f"{page_req.status_code}: Website could not be reached! Here is the url: {url}"
            )
            self.terminal_outputs.append(
                f"{page_req.status_code}: Website could not be reached! Here is the url: {url}\n"
            )
    ...
    def clean_data(self) -> None:
        ...
        self.df[binary_columns] = (self.df[binary_columns] == "Yes").astype(
            int
        ...
        self.terminal_outputs.append(
            f"Total number of properties (houses, apartments, ...): {self.df.shape[0]} \n"
        )
        self.terminal_outputs.append(
            f"Total number of features per property: {self.df.shape[1]} \n"
        )
